...
if ~isempty(opts.CAS)
    ...
    x=-1000:100:100000;
    [rho_ref,a_ref,T_ref,P,~,~,~] = dcrg.aero.atmos(x);
    [~,a_s,T_s,P_s,~,~,~] = dcrg.aero.atmos(0);
    CAS_ref = dcrg.aero.v.calibrated(M,P,P_s,a_s,1.4);
    CAS = opts.CAS;
    ...
    TAS = dcrg.aero.v.true(M,a,T,T_s);
else
    [rho,a,T,P,~,~,~] = dcrg.aero.atmos(opts.h);
    [~,a_s,T_s,P_s,~,~,~] = dcrg.aero.atmos(0);
    CAS = dcrg.aero.v.calibrated(M,P,P_s,a_s,1.4);
    TAS = dcrg.aero.v.true(M,a,T,T_s);
    h = opts.h;
end
